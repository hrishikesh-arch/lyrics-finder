...
import { Youtube, Disc, Music2, Copy, Check, Mic2, Play, Pause, RotateCcw } from 'lucide-react';
  ...
  return (
    ...
    >
            ...
            <div className="flex gap-2">
              ...
              {syncedLyrics ? (
                <button
                  ...
                  className={`flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 font-bold rounded-full transition-all backdrop-blur-md ${mode === 'karaoke' ? 'bg-purple-600 text-white shadow-lg shadow-purple-500/30' : 'bg-white/10 hover:bg-white/20 text-white'}`}
                >
                  <Mic2 size={18} />
                  <span className="text-xs tracking-wide">KARAOKE</span>
                </button>
              ) : (
                <button
                  disabled
                  className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 bg-white/5 text-white/30 font-bold rounded-full cursor-not-allowed"
      ...
      <div className={`w-full md:w-2/3 p-10 md:p-14 rounded-3xl shadow-2xl min-h-[60vh] max-h-[80vh] relative overflow-hidden flex flex-col ${theme.glass}`}>
        ...
        {/* Karaoke Controls */}
        {mode === 'karaoke' && (
          <div className="flex items-center justify-between mb-8 pb-6 border-b border-white/10 z-20">
